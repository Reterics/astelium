...
const ContractModal = ({
  ...
  const variableKeys =
    template.fields ||
    (template.content?.match(REGEXPS.variable.regexp) || []).map(
      (key, index) => {
        ...
        return {
          ...
          name: key.substring(
            startLength,
            key.length - startLength - REGEXPS.variable.end.length + 2
          ),
          validation: null,
        };
      }
    ...
    variableKeys.reduce(
      (out, current) => {
        if (out[current.name] === undefined) {
          ...
          out[current.name].startsWith('data:image')
        ) {
          out[current.name] =
            '<img src="' +
            out[current.name] +
            '"  alt="Signature" style="width: 150px; display: inline-block; "/>';
        }
        return out;
      },
      {...data}
  ...
  return (
    <Modal title={'Contract Modal'} onClose={close}>
      <form
        ...
        className='flex flex-row max-h-[90vh] min-w-[90vw]'
      >
        <div className='flex-col w-full p-2 no-print'>
          {variableKeys.map((templateField, index: number) => (
            <div className='flex text-right mt-2' key={'input_' + index}>
              <label
                htmlFor={`field-${templateField.name}`}
                className='w-36 text-sm text-zinc-500 text-right content-center pe-3'
              >
                {templateField.name}:
              </label>
              {templateField.type === 'Signature' ? (
                ...
                >
                  {data[templateField.name] && (
                    <img
                      src={data[templateField.name]}
                      className='w-full h-full bg-white'
                      alt='Signature'
                    />
                  )}
                  {!data[templateField.name] && (
                    <div className='w-full align-center'>Sign here</div>
                  )}
                </button>
              ) : (
                <input
        ...
        <div className='flex-col w-full h-full'>
          <div
            className='bg-white text-black a4-print printable'
            ref={targetRef}
      ...
      {signature && (
        <div className='fixed flex flex-row left-0 top-0 p-20 w-full h-full justify-center backdrop-blur-sm'>
          <div>
