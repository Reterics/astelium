...
const LineChart: React.FC<LineChartProps> = ({
  ...
  useEffect(() => {
    ...
    const yScale =
      scaleType === 'log'
        ? d3.scaleLog().domain([1, d3.max(data, (d) => d.count) || 10]).range([innerHeight, 0])
        : d3.scaleLinear().domain([0, d3.max(data, (d) => d.count) || 10]).nice().range([innerHeight, 0]);
    ...
    svg.on('mousemove', function (event) {
      ...
      const nearest = data.reduce((prev, curr) => (Math.abs(curr.timestamp - xDate.getTime()) < Math.abs(prev.timestamp - xDate.getTime()) ? curr : prev));
      const nearestX = xScaleRef.current(new Date(nearest.timestamp));
      const nearestY = yScaleRef.current(nearest.count);
  ...
  }, [data, width, height, themeColor, backgroundColor, zoomLimit, scaleType, zoomType, mouseLine, onMouseMove, onMouseOut, onMouseOver]);
