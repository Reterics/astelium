...
const TreeMapVisualization: React.FC<TreeMapProps> = ({
  ...
  const drawTree = useCallback((node: any) => {
    ...
    const nodes = g
      .selectAll("g")
      .data((node.children || node.leaves()) as HierarchyRectangularNode<HierarchicalData>[])
      .enter()
      .append("g");
    ...
    nodes
      .append("rect")
      .attr("class", "parent")
      .attr("transform", (d) => `translate(${xScale.current(d.x0)},${yScale.current(d.y0)})`)
      .attr("width", (d) => xScale.current(d.x1) - xScale.current(d.x0))
      .attr("height", (d) => yScale.current(d.y1) - yScale.current(d.y0))
      .attr("fill", assignColor)
      .attr("stroke", "#fff")
  ...
  return (
    <div className="relative">
      ...
      <svg ref={svgRef} width={width} height={height} className="bg-gray-100 rounded shadow-lg" />
    </div>
  );
};
