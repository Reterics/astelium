...
const SunburstChart: React.FC<SunburstChartProps> = ({
  ...
  useEffect(() => {
    ...
    const root: SunburstHData = d3
      ...
      .size([2 * Math.PI, hierarchy.height + 1])(hierarchy) as unknown as SunburstHData;
    root.each((d) => (d.current = d));

    const color = d3.scaleOrdinal(d3.quantize(d3.interpolateRainbow, (hierarchy.children?.length||0) + 1));
    ...
    function clicked(event: MouseEvent, p: SunburstHData) {
      ...
      path.transition(t)
        ...
        .filter(function (d) {
          return !!((this as SVGSVGElement)?.getAttribute("fill-opacity") || arcVisible(d.target))
        })
