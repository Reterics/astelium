...
const getDateKeys = (data: any[]) => {
  return Object.keys(data[0] || {}).filter(
    (key) =>
      data[0]?.[key] instanceof Date || !isNaN(Date.parse(data[0]?.[key]))
  );
};
...
const getGraphOptions = (
  ...
  LineChart: [
    ...
    },
    {
      ...
      label: 'Mouse Line',
      type: 'select',
      options: ['target', 'horizontal', 'vertical', 'none'],
    },
  ],
  ScatterChart: [
    {
    ...
    },
    {
      ...
      type: 'select',
      options:
        options.xAxisType === 'time' ? getDateKeys(data) : getNumericKeys(data),
    },
    {
      key: 'yAxisType',
      label: 'Y Axis Type',
    ...
    },
    {
      ...
      type: 'select',
      options:
        options.yAxisType === 'time' ? getDateKeys(data) : getNumericKeys(data),
    },
  ],
  SunburstChart: [
    {
  ...
  return (
    <div className='flex flex-col w-full h-full bg-zinc-100 text-zinc-700 p-4 rounded-md shadow-md'>
      <div className='flex items-center justify-between w-full space-x-4 border-b border-zinc-300 pb-2 mb-3'>
            ...
            opt.type === 'select' ? (
              <SelectComponent
                ...
                handleFilterChange={(_col, value) =>
                  handleOptionChange(opt.key, value)
                }
              />
            ) : (
        ...
        <h2 className='text-lg font-semibold text-zinc-800 flex-shrink-0'>
          {title}
        </h2>
      ...
      <div
        ref={containerRef}
        className='bg-white rounded-b-md shadow flex-grow'
      >
        <Component
          data={data}
          {...options}
